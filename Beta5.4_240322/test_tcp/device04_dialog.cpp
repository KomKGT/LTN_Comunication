...
QString Device04_dialog::OkBTN() //ใช้
{
    return status_button_state;
}

void Device04_dialog::on_buttonBox_accepted() //ใช้
{
    ...
    m_rtu.setComPort(ui->ComPort_Combobox->currentText());
    m_rtu.setSlave(ui->slave_line_edit->text());
    m_rtu.setBuadRate(ui->baudrate_combo_box->currentText());
    m_rtu.setCount(ui->Count_lineEdit->text());
    m_rtu.setReg(ui->Reg_lineEdit->text());
    m_rtu.setParity(ui->parity_combo_box->currentText());
    m_rtu.setDataBits(ui->data_bits_combo_box->currentText());
    m_rtu.setStopBits(ui->stop_bits_combo_box->currentText());
...
Property_RTU_DV4 Device04_dialog::rtu() const //ใช้
{
    return m_rtu;
}

void Device04_dialog::setRtu(const Property_RTU_DV4 &rtu)   //ใช้
{
    m_rtu = rtu;

    ui->ComPort_Combobox->setCurrentText(m_rtu.getComPort());
    ui->baudrate_combo_box->setCurrentText(m_rtu.getBuadRate());
    ui->slave_line_edit->setText(m_rtu.getSlave());
    ui->parity_combo_box->setCurrentText(m_rtu.getParity());
    ui->data_bits_combo_box->setCurrentText(m_rtu.getDataBits());
    ui->stop_bits_combo_box->setCurrentText(m_rtu.getStopBits());
    ui->Reg_lineEdit->setText(m_rtu.getReg());
    ui->Count_lineEdit->setText(m_rtu.getCount());
}
